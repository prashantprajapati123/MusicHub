...
class BeatportDiscography_shortcode {
	...
	var $plugin_version = '138'; // version 1.3.8
	...
	*/
		...
		if($feed == 'artist' || $feed == 'label' ){
			...
			if($items == 'release'){
				...
				for ($i = 0; $i < count($dataArray['results']); $i++){
					...
					$output .= '<img src="' . str_replace("{hq}", "_hq", str_replace("{w}", "60", str_replace("{h}", "60", $dataArray['results'][$i]->dynamicImages->main->url)))  . '"/>' . PHP_EOL;
					$output .= '</a>' . PHP_EOL;
					$output .= '</div>' . PHP_EOL;
	...
	function output_html( $atts, $content = null )
	{
		...
		if( isset($atts['perpage']) && ($atts['perpage'] >= 1 && $atts['perpage'] <= 150)){
		...
		if(isset($atts['feed']) && $atts['feed'] == 'artist'){
			$url .= '';
			$qrystring = '?facets=performerName:' . str_replace(' ', '+', trim($atts['artist'])) . '&sortBy=publishDate%20desc&perPage=' . $perpage;
			
		}elseif(isset($atts['feed']) && $atts['feed'] == 'label'){
			$url .= '';
			$qrystring = '?facets=labelName:' . str_replace(' ', '+', trim($atts['label'])) . '&sortBy=publishDate%20desc&perPage=' . $perpage;
