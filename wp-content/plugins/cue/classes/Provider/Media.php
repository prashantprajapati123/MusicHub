...
class Cue_Provider_Media extends Cue_AbstractProvider {
	...
	public function filter_audio_shortcode_attributes( $atts, $defaults, $user_atts ) {
		...
		$atts['categories'] = '';
		if ( isset( $user_atts['categories'] ) ) {
			$atts['categories'] = $user_atts['categories'];
		}
		/******added by vinay end  *****/
	...
	public function filter_audio_shortcode_html( $html, $atts, $audio ) {
		...
		/*** added by vinay start****/
		if ( empty( $atts['categories'] ) && ! empty( $meta['categories'] ) ) {
			$atts['categories'] = $meta['categories'];
		}
		/**** added by vinay end *********/
		...
		$html .= sprintf(
			...
			wp_json_encode( array(
				...
				'categories' => esc_html( $atts['categories'] ),
